...
apt-get update                             || exit $?
apt-get -y install openjdk-7-jdk           || exit $?
...
dpkg -i elasticsearch-1.4.2.deb             || exit $?
dpkg -i logstash_1.4.2-1-2c0f5a1_all.deb    || exit $?
...
cp /vagrant/files/elasticsearch.yml /etc/elasticsearch/  || exit $?
cp /vagrant/files/xdata.conf /etc/logstash/conf.d/       || exit $?
